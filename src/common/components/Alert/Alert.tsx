...
const Alert: React.FC<AlertProps> = ({alertType, onDismiss, children, ...props}) => {

    const determineAlertColor = (alertType: AlertTypes) => {
        if (alertType === AlertTypes.MESSAGE) return AlertColors.INFO;
        if (alertType === AlertTypes.FAILURE || alertType === AlertTypes.CRITICAL) return AlertColors.DANGER;
